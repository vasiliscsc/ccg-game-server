...
**Goal:** Move a board minion back to its owner's hand as a card.

**Files (create):** `Actions/ReturnToHandAction.cs` (`{ required string MinionId; string? SourceId }`), `Handlers/ReturnToHandHandler.cs`, `Events/CardEvents.cs` add `CardReturnedToHandEvent { string PlayerId; Card Card }`. **Test:** `Scenarios/BounceScenarios.cs`.

**Handler:** remove minion from board (unregister behaviour); reconstruct a `Card` from its `CardId` (fresh base card — buffs/damage lost, the canonical bounce rule; document); add to owner's hand (respect hand cap → if full, the minion is destroyed instead, emit appropriate events); emit `CardReturnedToHandEvent`.

**Scenario tests:**
- *Bounce resets:* a buffed/damaged minion bounced → returns as a fresh base card in hand; board slot empty.
- *Full hand → destroyed:* hand full → bounce destroys the minion instead (define & test).
- *No deathrattle on bounce:* deathrattle does not fire (not a death).
