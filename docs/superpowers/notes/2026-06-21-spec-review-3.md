...
- **G8 ⏸ OPEN — presented, NOT applied (deferred to session 20 at the user's request).** A ruling *was presented*: (a) a response **may** open a `PendingChoice` (the responder's own Discover/modal); `pendingChoice` + `pendingIntervention` coexist as orthogonal §1 fields, `phase → PendingChoice` with the held action parked, not barred by depth-1, and it resolves **before** the intervention loop resumes; (b) the **pre-halt death rule must NOT fire for a response-nested `PendingChoice`** (`pendingIntervention != null` ⇒ inherits the reaction's deferred-death state) — else a ⑥′ reaction whose response card opens a Discover would pre-settle the very mortally-wounded subjects the ⑥′ window relies on lingering. **⚠ User wants to revisit the Discover/Modal mechanic FIRST** — re-derive *why* the pre-halt death rule was spec'd (amendment B), its implications, and its importance — and only *then* answer G8. **Do NOT apply (b) until that review.** (The structural half (a) looked uncontroversial; the death-timing carve-out (b) is the part to re-examine.)
...
### G8 — [HOLE / L] `PendingChoice` opening *inside* an intervention response is unaddressed — ⏸ OPEN (presented session 19; deferred — revisit Discover/Modal + pre-halt-death-rule rationale first)
