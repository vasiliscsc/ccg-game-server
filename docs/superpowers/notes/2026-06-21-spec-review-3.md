...
Nothing walked yet. Suggested order at the bottom. Headliners: **G1** (Combo self-counting) and **G2** (`SpellCastEvent` can't read the cast card's cost) are the two with real gameplay teeth.

---

## A. Findings

### G1 — [BUG / M] Combo counts the card that triggers it (off-by-one from the commit/resolve split)
...
### G2 — [HOLE / M (H if many cost-gated cast cards)] `SpellCastEvent` can't evaluate the cast spell's cost/stats — and `CardPlayedEvent` can
...
### G3 — [DRIFT / L] `MinionOnBoard` field block omits the tribe fields it's said to own
...
### G4 — [DRIFT / L] `TakeControlAction` says "reject `BoardFull`" but, as an effect-issued action, should **fizzle**
...
### G5 — [DRIFT / L] Orphaned intervention events: `InterventionPlayedEvent` (and maybe `InterventionWindowClosedEvent`)
...
### G6 — [HOLE / L] Enrage's conditional-buff storage is under-specified
...
### G7 — [HOLE / L] Trigger-registration epoch for **setup-equipped** artifacts is unspecified
...
### G8 — [HOLE / L] `PendingChoice` opening *inside* an intervention response is unaddressed
