...
- **#35 LOGGED (not resolved here).** The "AttackDeclaredEvent is a renderer convenience" correction surfaced that a **cancelled** attack currently keeps Stealth + budget (break is at ④, post-③′). User intent = declaring reveals even on interception. Logged as findings-doc **#35**; to be walked next via the #34-style `AttackAction` commit/resolve split.
