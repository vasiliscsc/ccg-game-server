...
**⏳ RESUME HERE — D1–D4** (confirm-on-record design notes — *none applied yet*; an earlier D1 edit was drafted and rejected so the F16/F18/F19 questions could be raised first). D1 = active player can't reactively save self on own turn (by design); D2 = `SpellCastEvent` fires at commitment even if the spell is then countered; D3 = Reborn model simplification (subsumed by F5 — no edit needed); D4 = press cap of 3 is a balance lever. These need only optional clarifying one-line notes (D1/D2) — no behavior change.
