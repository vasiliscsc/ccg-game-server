...
**Still deferred (game-feel, does not block this lock):** the **marked-for-destruction scope** of dying-window saves (a destroy-marked minion cannot be healed/inverted back above 0). *(The former secret-bundle deferrals — auto flavour, zone, constraints, telegraph, cost timing, identity — are all resolved by this subsection. Batching: §4 "Batching".)* See the borrow-list note.
