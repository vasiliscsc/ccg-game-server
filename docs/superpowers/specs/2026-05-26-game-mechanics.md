...
- **Battlecry** resolves synchronously inside the **card-resolution pipeline (`ResolveCardAction` ④** — not the now-commit-only `PlayCardAction`, per #34; F11) and may pause for targeting via `PendingChoice`, *before* the minion's persistent triggers go live.
