...
| Action | Key fields |
|---|---|
| `PlayCardAction` | playerId, cardId, targetId? — playing a **sigil** outside a response window = **INSCRIBING** it (§3 "Sigils"): same cost gate (`effectiveCost ≤ mana` — pay-on-inscribe, the trigger fires free later), ③ additionally rejects `InscriptionSlotsFull` (row of 3) / `SigilAlreadyInscribed` (duplicate definitionKey); the ④ handler moves the card hand → `inscriptions`, registers its reactive trigger (zone-entry, `birthEpoch` stamped), counts the play (`cardsPlayedThisTurn++`), and emits the owner-directed `SigilInscribedEvent` + public thin `InscriptionAddedEvent` **instead of** `CardPlayedEvent`/`SpellCastEvent` (identity-hidden). Playing a sigil from hand *inside* a window goes via `SubmitInterventionAction` (an **invocation**) instead. |
...
| `InvertTargetAction` | targetId, sourceId? — toggles `isInverted` and performs the **stat trade (pinned 2026-06-11): `currentHealth` and `attack` trade places, on aura-exclusive values; auras re-apply after.** ④ handler, in order: **(1)** strip auras — `A = attack − auraAttackBonus`, `H = currentHealth − auraHealthBonus`, damage `D = (auraless maxHealth) − H`; the handler never touches `aura*` fields. **(2)** Orientation flip: the base stat pair **swaps systemically** (`baseAttack ↔ baseHealth` — the definition's inverted section defines *effects/triggers*, never its own stat line) and every enchantment's deltas swap (`attackDelta ↔ healthDelta`) — required for self-consistency: it makes the new auraless `maxHealth` equal exactly `A`. **(3)** Trade: `currentHealth ← A` (the minion lands at **full flipped health** — damage does not persist as damage…) and the attack side absorbs the damage via a system `StatModifier { attackDelta: −min(D, auraless maxHealth), sourceId: "inversion" }` (…it persists as **reduced attack**: new auraless attack = `max(0, H)` = the old `currentHealth`, floored at 0 — derivation model intact, no stored negative attack). **(4)** Emit `MinionInvertedEvent`; the card-handler swaps to the inverted section's triggers (On Invert fires); stage ⑥ then re-applies auras on the new orientation (aura *grants* don't flip — a +1/+1 aura re-applies as +1/+1). **Consequences, accepted:** re-inverting an unhealed minion re-trades the penalty back as a health delta (the wound returns — an R2-saved minion re-doomed if re-inverted before healing); Silence clears the inversion-damage modifier like any enchantment (silencing forgives the traded damage). **R2 × lethal falls out:** a mortally-wounded minion (`H ≤ 0`) inverted in the dying window returns at full flipped health with 0 attack, and survives the ⑦ wave iff `A > 0`. (**Health-domain dooms only:** inverting a destroy-*marked* minion executes the trade but the mark survives — it still dies at the settle; §2A `DestroyMinionAction`.) **Card-in-hand inversion** is the same flip minus the trade: `isInverted` toggles, base pair swaps, `modifiers` deltas swap — no `currentHealth` exists. |
| `UnInvertTargetAction` | targetId, sourceId? — the **same operation** as `InvertTargetAction` (the trade is symmetric), differing only in the required prior state: invert requires `isInverted == false`, un-invert requires `true`; on a mismatched target the action **no-ops** (no event). |
...
**Directed events (2026-06-10, intervention visibility).** Most events broadcast to both players' wire streams (and spectators/replay). A few are **directed**: delivered only to one player's wire and absent from the opponent's and the spectator stream — the pattern the mulligan events already used ("each with only their own options"). Directed in v1: `MulliganStartedEvent`, `ChoicePromptEvent`, `InterventionPromptEvent`. The library marks directedness on the event type; the Game Server's per-player fan-out honors it (the split exists precisely so a prompt's private payload — Discover options, intervention candidates — never crosses to the other wire). `eventId`s are assigned globally at append, so directed filtering never renumbers anyone's stream.
...
| `CardPlayedEvent` | playerId, card, targetId? — **not** emitted for an inscribe play (identity-hidden; the §2B sigil events replace it — §3 "Sigils") |
...
| `MinionMortallyWoundedEvent` | minionId, sourceId — fires the instant a minion enters **health-domain** pending-death (`currentHealth ≤ 0` — from damage, or from a buff/aura loss collapsing `maxHealth` to ≤ 0), **before** the death-wave settle removes it; distinct from `MinionDiedEvent` (Phase-1 removal). The hook for dying-window reactions — e.g. a **save-from-lethal** intervention (the "R1" avenue) or an R2 inversion save. **A destroy-mark does NOT fire this event (narrowed 2026-06-12, option A):** fiat dooms are unsavable, so no dying window may open to bait a futile save — a marked minion is silent until `MinionDiedEvent` at the settle; its counterplay window was the `DestroyMinionAction`'s own ③′ declaration. (No `cause` discriminator — and with marks excluded, every remaining cause is a health crossing, genuinely indistinguishable-by-need; see Mana/Hero note below.) |
...
| `InterventionWindowOpenedEvent` | respondingPlayerId, timeoutSeconds — **public thin form**: a window exists + its clock. The acting player thereby learns *a* matching reactive card exists for this action (the accepted contingent-window tell — Arena/Master-Duel camp, decided 2026-06-10) but never which; UX-layer masking is a server option, no engine change |
...
| **hand** (sigil) | the `DrawCard`/`GiveCard` handler (entering hand), dropped on play/discard/return | opens a **set-valued, looping intervention** window for the owner (play one / skip; re-opens on each play) — playing one is an **invocation** | **this amendment** |
...
- **Windows stay contingent** (they open only when candidates exist) with a **thin public event** alongside the responder-directed prompt (§2B): the acting player learns *a* matching reactive card exists — the Arena/Master-Duel tell, accepted deliberately over LoR-style structural windows (which would pause every action all game). Masking the tell is a server/UX option, not engine machinery.
...
- **Actor-attribution firing rule** (replaces any turn-based rule): an inscription **never fires on events caused by its own owner's action** — skip iff the causing action is attributed to the host's owner (player actions → the submitter; effect actions → the controller of the attributed source, §3 "Source Attribution"; neutral-sourced actions are attributed to no one and can spring **both** players' inscriptions). On an ordinary turn this is indistinguishable from HS's "opponent's turn only" — but it correctly lets *my* inscription punish an opponent's **invocation during my turn** (windows never change `TurnState.activePlayerId`; a window is a suspension inside the actor's turn), and a player's own deathrattles resolving off-turn never spring their own inscriptions.
...
- **Dispatch slot:** inscription-hosted triggers fire **inside their owner's dispatch group, after that owner's artifacts**, in inscription order (`inscriptions[0..2]`) — the artifact precedent extended one slot (§3 `IEventBus`). Auto-chains (my inscription's response springing theirs, springing mine…) need no depth rule: each fire consumes its card and the rows cap at 3, so chains terminate structurally.
- **Visibility:** existence public, identity hidden — inscribing emits the owner-directed `SigilInscribedEvent` + public thin `InscriptionAddedEvent` *instead of* `CardPlayedEvent` (§2B); full public reveal only at fire time. The armed-state telegraph question is thereby resolved: the zone shows N face-down cards, HS-style (full hiding is unachievable anyway — the mana delta is public). Hand-held sigils need no telegraph rule: they are ordinary hidden hand cards, and the contingent-window tell is already accepted.
...
2. If reactive triggers fired and their host **auto-resolves** (an **inscription** — §3 "Sigils"; choices degrade to random, actor-attribution skip applies), the response resolves inline here (no halt). If matching **hand cards** offer a player response, the engine **suspends** the action — stores it as `pendingIntervention.heldAction`, records the responder's matching + affordable cards with their computed targets in `candidates` (§1 `InterventionCandidate`), sets `phase = PendingIntervention`, and opens the **set-valued** window (§4 "PendingIntervention Interruption") — emitting the responder-directed `InterventionPromptEvent` (cause = the **held action verbatim**; §3 visibility) and the public thin `InterventionWindowOpenedEvent` (§2B).
...
1. The engine gathers the responder's reactive hand cards whose triggers matched **this action's** events and that are **affordable** (`effectiveCost ≤ mana`), records them — each with its computed targets — in `candidates` (§1 `InterventionCandidate`), and opens **one set-valued** `PendingIntervention` window (`phase → PendingIntervention`, `heldAction = null`) — emitting the responder-directed `InterventionPromptEvent` (cause = `causeEventIds[]`, refs to this action's matched events, already on the responder's wire from ⑤; §3 visibility) and the public thin `InterventionWindowOpenedEvent` (§2B). Each card's offered targets are its matched subjects from this action ∩ its declared `(selector, cardinality)` family.
...
A window **opened by live reactive triggers on hand cards** that matched (§3 "Reactive Triggers, Interventions & Interception") — never a general engine rule. It is **set-valued** (offers *all* the responder's matching + affordable cards in `candidates`, each with its engine-computed targets — §1 `InterventionCandidate`) and **loops** by re-opening after each play; it resolves one of two ways, by which phase the triggers hooked. (Throughout: `phase → PendingIntervention`; `candidates` lists the offer; the responder-directed `InterventionPromptEvent` + public thin `InterventionWindowOpenedEvent` go out (§2B); timeout timer starts; only `SubmitInterventionAction` from the responding player is accepted — **exempt** from the ③ turn-ownership check, its cost paid from the responder's `mana` (their next-turn pool, pre-loaded at turn-end — Turn Lifecycle); a **play** re-opens the window over the remaining offer, a **skip / timeout** ends the loop; timeout = an injected skip, logged as an input per Item 13; `phase → InProgress` at the end.)
