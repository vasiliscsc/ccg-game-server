...
### ⏹ SESSION STOP (2026-06-03, end of session 2)
...
- **D — Secrets / armed reactive triggers** *(natural next pick — R1 routed its window-opening + batching questions here; also resolves "what opens a dying-window intervention").*
- **A — Predamage** — an incoming-damage modification hook (armor, double-damage, damage reduction/caps); our `DealDamageAction` only has source-side hooks + Divine Shield.
- **C — Play-requirements** — non-target board preconditions ("requires ≥2 minions", "requires a damaged friendly target") beyond the target selector.
