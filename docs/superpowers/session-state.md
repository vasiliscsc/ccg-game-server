...
**▶ RESUME AT ITEM 12 — Targeting strategy registry.** Items 1–11 are resolved (✅ below); 12 and 13 remain, then end-of-pass plan reconciliation. Session of 2026-06-01 closed here after walking Items 6–11. Workflow unchanged: present analysis + recommendation, the user picks, then amend spec + record `✅ DECISION` (+ Plan impact) in the borrow-list note and tick the tracker here.

Item 12 is **substantive** (not a quick reject like 10/11). Note recommends *Adapt*: an `ITargetSelector` interface (`SelectorId`, `Select(EffectContext) → TargetSelection`) replacing the old enum-switch, where `TargetSelection` is `Immediate(EntityId[])` or `RequiresChoice(StartChoiceAction)`. Forks to frame next time:
- **Selector vs validator** — keep distinct: Item 5 deliberately decoupled `GetLegalActions` from a target *enumerator* (it reuses only the *validator*). So `ITargetSelector` is for *resolving* an effect's targets, not for enumerating legality.
- **`RequiresChoice` path** threads into the already-locked `PendingChoice` mechanism (mid-effect targeting pause; continuation serialized into `PendingChoice.context`).
- **RNG access (the Item 6 parked question)** — does a selector receive `IRandom` via `EffectContext` for random-target effects? Remember Item 6 locked that randomness is consumed only in stage-④ handlers; a selector running inside effect resolution may need the per-action `IRandom`. Decide here.
- **Multi-target ordering** — ordered vs unordered `EntityId[]` (note's open question).
- Apply the disposition test (reject / Unaddressed-Features / future-epic-deferral / spec) established in Item 10.
...
12. ⏳ **NEXT** — Targeting strategy registry
