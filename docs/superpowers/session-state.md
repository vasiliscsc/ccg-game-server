...
**Remaining pre-implementation work:** the **end-of-pass plan reconciliation** (all `Plan impact:` lists → epic/ticket files; weapon-scope deletion + artifact re-scope + sigil additions + the library-seam constraint + `HasTribe` + the `InvertTargetAction` semantics) → then **Epic 01 / T1.1**. The sole remaining deferred-by-design item: **marked-for-destruction scope** of dying-window saves (has a home in §3).
