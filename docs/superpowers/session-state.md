...
**▶ RESUME (session 19): D1–D4** (confirm-on-record — D1 own-turn self-save by design, D2 `SpellCastEvent`-on-countered-spell, D3 Reborn note subsumed by F5, D4 press-cap balance lever; my rec = add D1+D2 one-liners, none for D3/D4). **Then spec-review-2 is COMPLETE → end-of-pass plan reconciliation** (must fold in F1–F17 + F19) → **Epic 01 / T1.1**.

**Uncommitted:** session-18 spec edits (F19 simplification + Unaddressed entry) + findings-doc updates + this block are in the working tree, on top of the still-uncommitted session-17 edits (now reverted/reworked). Not committed — left for the user's call (the prior convention commits design-doc edits directly to main).
