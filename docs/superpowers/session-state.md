...
**Still-open holes** (#3 + follow-on done; #1 done): **#2** fatigue not in the data model (no counter on `PlayerState`, no action/event, yet `HeroMortallyWoundedEvent` lists `fatigue` as a cause); **#4** — now just the **Freeze `frozenOnTurn` thaw rule** (Stealth-on-attack half closed by hole #1). Resume by picking one, or continuing the open Q/A.
