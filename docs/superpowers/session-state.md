...
### Session 19 (2026-06-22→23): SPEC REVIEW #3 walk — G1–G7 applied, G8 OPEN

Walked `notes/2026-06-21-spec-review-3.md` one-by-one (confirm-each cadence). **G1–G7 done; G8 deferred at the user's request.** Per-finding walk-progress + a new **"Plan impact (fold into reconciliation)"** section live in the findings doc.

- **G1 ✅** Combo off-by-one: predicate stated as **`cardsPlayedThisTurn ≥ 2`** (not `> 0`) — §1:79 off-by-one note + §3:692 Combo predicate. `ComboTriggeredEvent` unchanged.
- **G2 ✅** (user picked the clean fold) `SpellCastEvent` now carries the **full `card`** (mirrors `CardPlayedEvent`), threaded via a `card` snapshot on `ResolveCardAction`; the action's old separate `cardId`/`definitionKey` **dropped** (subsumed by `card.id`/`card.definitionKey`). §2A:318/319, §2B:462. `GraveyardSpell` deliberately untouched.
- **G3 ✅ NO-OP (reviewer miss)** — the tribe fields are **already** in the `MinionOnBoard` block (§1:101–104); finding's premise false. No edit.
- **G4 ✅ APPLIED w/ CORRECTION** — finding said "reject→fizzle," but a **user clarifying question** ("fizzle the effect or the minion?") triggered HS verification: a take-control onto a **full board DESTROYS the target** (Deathrattle fires as the *original* owner's — [hearthstone.wiki.gg/Take_control](https://hearthstone.wiki.gg/wiki/Take_control)). So §2A:351 now enqueues `DestroyMinionAction` (the `ReturnToHandAction` #21 full-hand pattern), **not** fizzle. `BoardFull` code stays (player minion-play ③). *F18/D2 lesson reaffirmed — verify HS, not the finding's framing.*
- **G5 ✅ DELETE BOTH** — `InterventionPlayedEvent` + `InterventionWindowClosedEvent` removed (§2B). **Gated on a full invocation walkthrough** (user-requested before deleting): proved "played in response" = `CardPlayedEvent`(+`SpellCastEvent`)/`SigilRevealedEvent`, and window-close is inferable in every close mode. Borrow-list note's old "stays public" line annotated SUPERSEDED.
- **G6 ✅ DISSOLVED (user's leaner call)** — dropped the **`Enrage` keyword + `EnrageStateChangedEvent`** entirely (§3:581/§2B:429); Enrage is now a **self-conditional `IAura`** (a `Calculate` returning a self-targeting, condition-gated `AuraEffect`; bonus rides the ordinary `aura*Bonus` channel). Added an `IAura` authoring note **+ convergence caveat** (per "1+2": a self-referential aura whose *output feeds its own predicate* is non-convergent/undefined; Enrage is safe — predicate on health, output on attack, gap #13-invariant). **User corrected my Example B mid-walk** (a co-present health aura keeps the damage gap, so no snapshot rule is needed — that idea was decoupled as a separate general-`IAura` matter). Sizable plan impact: Epic 08 **T8.9** reworked (keep On-Damage-Taken trigger, drop `EnrageKeyword.cs`/event; Enrage → IAura scenario test); Epic 16 Silence×Enrage note reframed; inversion-v2 note flagged (Enrage no longer a keyword).
- **G7 ✅** setup-equipped starter artifact triggers/auras pinned to **`birthEpoch = 0`** at Match Setup step 1 (§4:1149) — moot for the v1 active-only starter, pins the rule for a future passive/hero artifact.
- **G8 ⏸ OPEN — PRESENTED, NOT APPLIED (user deferred to session 20).** Ruling presented: (a) a response **may** open a `PendingChoice` (responder's own Discover/modal; `pendingChoice`+`pendingIntervention` coexist as orthogonal §1 fields; not barred by depth-1; resolves before the intervention loop resumes) — structural half, uncontroversial; (b) **the pre-halt death rule must NOT fire for a response-nested `PendingChoice`** (`pendingIntervention != null` ⇒ inherits the reaction's deferred-death state) — else a ⑥′ reaction whose response card opens a Discover would pre-settle the mortally-wounded subjects the window relies on lingering. **⚠ User wants to revisit the Discover/Modal mechanic FIRST** — re-derive *why* the pre-halt death rule was spec'd (amendment B), its implications, and its importance — and only *then* answer G8. **Do NOT apply (b) until that review.**

### Session 23 (2026-06-28): SPEC REVIEW #4 raised (two-lens) + walk STARTED — A2/A3 applied

A user-requested **fourth** read-only spec review, this time through **two explicit lenses**: (1) senior systems architect (contradictions / generalizations-speced-as-exceptions / under-specs), (2) game designer (limitations, fun-killers, consequences that may not match intent). Findings written to **`notes/2026-06-28-spec-review-4.md`** — **9 findings: A1–A5 (architect) + GD1–GD4 (game designer)** + a "Checked and consistent" coverage section + triage order + Plan-impact. Then **started the walk** (present-tradeoff-+-recommend, reviews #1/#2/#3 cadence).

**Findings (headlines):**
- **A1 [M–H, UNDER-SPEC]** — the "resolve a window response *ahead of the pre-existing queue*" mechanism (the load-bearing save-before-settle guarantee) is **unnamed**, and the only front primitive `EnqueueFront` is annotated "death resolution only" (which itself looks stale — ⑦ runs at queue-empty). Fork: **nested drain** (rec) vs **front-priority queue**.
- **A2 [M, CONTRADICTION]** — §4 ③′ called `RespondInterventionAction`/`SubmitInterventionAction` "system actions," but they're player-submitted + phase-gated (line 1069 says system actions *bypass* Phase Guard). **A3 [L–M]** — `SubmitChoiceAction` omitted from the ③′-exemption → would open a spurious window resolving a Discover.
- **A4 [M, REDUNDANT-EXCEPTION]** — `AttackDeclaredEvent` is a per-type `*Declared` event contradicting `ActionDeclaredEvent`'s own "no `*Declared` taxonomy" principle, and double-declares with it at the same ③′. Fork: **drop (generalize)** vs **keep-but-demote to perf-sugar**.
- **A5 [L, UNDER-SPEC]** — ⑥ aura-recalc publishes `MinionMortallyWoundedEvent` but ⑤ is "the publish stage"; the originEpoch/dispatch/⑥′-gather of ⑥-published events is unpinned.
- **GD1 [M–H, FUN-RISK]** — the carry-mana model (no turn-start refresh) means an off-turn invoked intervention **permanently reduces your own next turn's mana** → hand-invokes cost ~double, self-mana-denial possible, asymmetric with pre-paid inscriptions, opaque to HS-trained players. Verify the cost curve is intended.
- **GD2 [M–H, FUN-RISK]** — unconditional decision-window flood, worst in combat: one attack = 3 actions × (③′+⑥′) = up to **6** windows; `skip-all` ≈ 3 prompts/attack; `autoSkipAll` all-or-nothing. The deferred "decision-window budget" Unaddressed entry undercounts ("one window per action"); DR1 keywords don't relieve combat.
- **GD3 [L]** — pending-death bodies linger longer than HS players expect; mechanic's fun rides on UI the engine can't enforce (client/content dependency). **GD4 [L, informational]** — heroes can't attack at all (weapons/hero-attack cut, session 9); confirm artifact row covers the aggressive design space.

**Walk progress this session:**
- **A2 + A3 ✅ APPLIED** (one shared mechanical fix, no fork): §4 ③′ step 1 rewritten to name two disjoint ③′-exempt classes — system *openers* (`Start*`, bypass Phase Guard) vs player-submitted, phase-gated *responses* (`Respond*`/`Submit*Intervention`/**`SubmitChoiceAction`**, NOT system); unifying property = "window/choice machinery, not a state-mutating board move." Cross-notes at §2A `SubmitChoiceAction` row + "Responder-initiated (off-turn)" header. Grep-clean.
- **A4 ⏸ clarification provided, decision deferred to session 24.** The user asked for the full attack action+event sequence before deciding. Walked it (recorded in the findings doc): every interception point is covered by the **general** `ActionDeclaredEvent` filtered by action type (`AttackAction` ③′ = redirect-the-attack; each `DealDamageAction` ③′ = prevent-the-hit), and the UI never receives *either* declaration event (both bus-only) — it renders from `AttackPerformedEvent` + two `DamageTakenEvent`s + status events + the public `InterventionWindowOpenedEvent`. **Conclusion supports DROP** (loses nothing on interception or render). User to confirm drop-vs-demote next session.
- **A1 ⏸ deferred to session 24** (user stopped for the day; fork unchanged).

**▶ RESUME PLAN (session 24):** finish the spec-review-4 walk — **A4** (confirm drop vs demote; walkthrough already supports drop), **A1** (nested drain vs front-priority), **A5** (mechanical ⑥-publish clause), **GD1 + GD2** (the two design calls worth deciding before content hardens), **GD3/GD4** (plan/UX notes). **THEN** the long-pending end-of-pass **PLAN RECONCILIATION** (fold every `Plan impact:` list — borrow list + Fireplace + holes + sigils/artifacts + spec-reviews #1 #1–#33 + #2 F1–F19/D1–D4 + #3 G1–G8/DR1–DR6 + **#4 A1–A5/GD1–GD4** + library-seam + stale-name sweeps — into `plans/2026-05-27-ccg-game-logic/`) → **Epic 01 / T1.1**.

**Git:** session-23 batch (A2/A3 spec edits + the new spec-review-4 doc + this state) committed + pushed to `origin/main`. Design-doc convention = work from `main`.

### ⏹ SESSION STOP (2026-06-28, end of session 23)

See the "Session 23" block directly above for the full record. **State:** spec-review #4 raised (9 findings A1–A5 + GD1–GD4, two-lens) and the walk STARTED — **A2 + A3 applied**; **A4** clarified via the attack-sequence walkthrough (supports drop) but the decision is the user's next session; **A1 deferred**; A5/GD1–GD4 not yet walked. Spec edits grep-clean. **Resume = finish the A4/A1/A5/GD walk → end-of-pass PLAN RECONCILIATION → Epic 01/T1.1.**

### Session 22 (2026-06-28): SPEC REVIEW #3 — DR5 + DR6 walked → REVIEW #3 COMPLETE

Walked the last two design recs. **Spec review #3 is now fully complete (G1–G8 + DR1–DR6).** Per-finding records + Plan-impact lines in `notes/2026-06-21-spec-review-3.md`.

- **DR5 ✅ APPLIED — alternate-win seam in ⑧, option A (registered `IWinCondition` list).** The user pushed back before locking — "how would ⑧ know all win conditions; does A hard-code them while B self-registers?" — which exposed my under-spec of A. **The clarification:** *both* options self-register; the real fork is **pull-at-settle (A) vs push-mid-cascade (B)**. **A is the `IAura` pattern applied to win-checking** — ⑧ iterates a *registered* list of `IWinCondition.Evaluate(state) → winnerId?` exactly as ⑥ iterates registered `IAura.Calculate`; ⑧ hard-codes nothing but "iterate the registry." A's win on **correctness-by-construction**: re-evaluating fresh at the settle inherits the same discipline ⑧ already uses for hero health (a condition undone by a pre-settle save correctly doesn't win), whereas B's `pendingWin` mark goes **stale** and must re-verify at the settle — collapsing into A's predicate. The artifact-charges quest decouples identically under both (trigger bumps `charges`; predicate reads `charges ≥ goal` = the HS quest model). **Applied:** §4 ⑧ rewritten (registered list; built-in hero-death entry; mutual-winner→draw = general mutual-lethal rule; pull-not-push rationale); new §3 `IWinCondition` subsection (after `IAura`; same registration shape; **v1 ships zero registered conditions** — seam exists so alt-win content is v2-on-a-fixed-seam, never settle-loop surgery); `IWinCondition` added to the §3 extensibility-layer enumeration (:522). Hero-death stays ⑧'s hardwired built-in, **not** an `IWinCondition`.
- **DR6 ✅ APPLIED — one-line note + plan obligation (user's pick).** Abort→void/`NoContest` unchanged (settled: a draw skews MMR). Added a sentence at the §4 mitigation paragraph marking **`MaxDeathWaves = 16` provisional** (a `GameConstant`, hence tunable; set high enough to bite only true runaways, not deep-but-finite *legal* cascades; revisit against real cascade-depth telemetry; the **card-load validator is the real guard**). Joins the playtest-watch list (DR2/DR3/DR4/DR6).

Style: held present-tradeoff-+-recommend; when the user challenged option A I re-derived it (the `IAura`-precedent reframe) rather than restate the recommendation — the challenge improved the spec text ([[feedback-design-fork-style]]).

### ⏹ SESSION STOP (2026-06-28, end of session 22)

**State:** **SPEC REVIEW #3 COMPLETE** — every finding G1–G8 (sessions 19–20) and every design rec DR1–DR6 (sessions 21–22) walked and applied/recorded. DR5 added the `IWinCondition` alt-win seam to §4 ⑧ + §3; DR6 marked `MaxDeathWaves` provisional. Spec edits grep-checked; findings doc Status → ✅ COMPLETE with full per-DR records + Plan-impact lines. **No design threads remain open.**

**▶ RESUME PLAN (session 23) — the end-of-pass PLAN RECONCILIATION (the one remaining pre-implementation task).** Fold **every** `Plan impact:` list into `plans/2026-05-27-ccg-game-logic/` (epic + ticket files + README), then begin **Epic 01 / T1.1**. Sources to sweep, in one pass:
- Borrow-list note (`notes/2026-05-28-old-project-borrow-list.md`) — Items 1–13 Plan-impact + the known new tickets (T4.8 abort telemetry, T8.10 `ITriggerCondition` lib, Epic-08 `ITargetSelector` lib, Bot-Support epic, Epic-01 `IRandom`/error-codes, tribe/keyword 4-field model, Epic-16 command-log replay, `IKeyword` collapse + Epic-07 re-scope) + the Fireplace-comparison amendments (keyword collapse, death cadence, point D, sigils, artifacts, hero-combat weapon-deletion + artifact re-scope).
- Spec-review #1 (`notes/2026-06-12-spec-review-findings.md`) #1–#33 Plan-impact.
- Spec-review #2 (`notes/2026-06-17-spec-review-2.md`) F1–F19 + D1–D4 Plan-impact (esp. F4 `IEntityPredicate`, F9 ChoiceType/ChoiceOption, F10 fizzle-RFG).
- Spec-review #3 (`notes/2026-06-21-spec-review-3.md`) **"Plan impact" section** — G1/G2/G4/G5/G6(sizable, Epic 08 T8.9 rework)/G8 + DR1 (envelope `suppressDeclaration`/`propagate`)/DR2/DR3/DR4/**DR5 (`IWinCondition` + ⑧ list)**/DR6 (card-load validator + provisional cap).
- Stale-name sweeps (old field names `canAttack`/`attacksAllowedThisTurn`/`originalCard`, weapon/hero-power vestiges) across plan + README.

**Git:** session-22 content batch (spec DR5/DR6 edits + findings-doc records + this state) to be committed + pushed to `origin/main`. Design-doc convention = work from `main`.

### Session 21 (2026-06-28): SPEC REVIEW #3 — DR walk (DR1–DR4 done; DR5–DR6 remain)

Walked the design-recommendation chapter (DR1–DR6) of spec-review #3, present-tradeoff-+-recommend cadence. **DR1–DR4 done; DR5–DR6 remain.** Full per-finding records + Plan-impact lines in `notes/2026-06-21-spec-review-3.md` ("Walk progress" + "Plan impact").

- **DR1 ✅ APPLIED — non-interceptable cards became a real keyword pair (the user reframed a "bool seam" into content).** Window-pacing fix = **`Uncounterable` / `Inexorable`** *play-phase* keywords (read at `PlayCardAction`, **not** on-board effective keywords; inert once resolved). Both treat **`{PlayCardAction, ResolveCardAction}` as one play-unit that skips ③′** (neither *prevented* nor *countered*). They differ only in reach over `ResolveCardAction`'s **enqueued** effects: `Uncounterable` = play-unit only (Battlecry/damage stays interceptable — Ice-Block works); `Inexorable` = propagates down the whole **own-effect spine** (nothing interceptable). **Suppression = NOT publishing `ActionDeclaredEvent`** (joins the `Start*`/`Respond*` system-action exemption); a `suppressDeclaration` flag threaded across commit→resolve unconditionally, onward to effects only under `Inexorable` (a `propagate` bit), **never inherited by triggered reactions** (the §3 `sourceId` discipline). **Post side (⑤/⑥/⑥′) untouched** — "you can't stop it," not "you can't answer its aftermath." Covers minions (Battlecry sits on the own-effect spine via F19; post-summon reactions still fire). Applied: spec §3 new "Non-interceptable actions" para, §4 ③′ step 1, §2A `PlayCardAction`/`ResolveCardAction` rows. Keyword flavor is the user's.
- **DR2 ✅ APPLIED — purpose reframe.** The "universal draw is OP" pitch collapsed: a *symmetric* tool creates **no** player-vs-player imbalance — the Warlock/fairness lesson does **not** transfer (the user corrected my misapplication). Residual = a milder *archetype tilt* (symmetric **access** ≠ symmetric **benefit** — draw favors grind/combo over aggro). The user reframed the tool's **purpose** as a **consistency / anti-variance bandaid**, which **flips the cost analysis**: under the flood reading a **mana cost is the *right* incidence** (a flooded player has spare mana by definition → the price self-targets exactly when the tool is wanted), vindicating the user's **base = 2** over my earlier "regressive" read. Resolution: keep the universal draw starter, **base cost → 2** (`2 + usesThisTurn`), **no** health cost; flood-vs-screw mechanism (plain draw vs loot/dig) + the grind-tilt = playtest-watch. Pure content (the starter is a definition). Applied: §1 `ArtifactOnBoard` starter cost note.
- **DR3 ✅ APPLIED — collapsed to content.** Neutral first-crack windfall: "move the spawn to turn-start" is a **non-fix** (turn-start-of-incoming = turn-end-of-outgoing, same boundary) — dropped. The contest **'boss' objective** (beefy health, both players chip it, claim on death) is **pure v2 content** on existing rails (`OnFire` already has the event §3:648, neutrals are attackable) — a high-`baseHealth` neutral + a death/damage-reaction trigger authored into `spawnPool`; reward → **the active player at the ⑦ that reaps it** (user's pick; coarser than last-hit-source but needs no event-source binding). v1 lever = **curate `spawnPool`** (content). Applied: a DESIGN-REMINDER note on the `spawnPool` field (§1 NeutralZoneConfig). Optional future library nicety: a general "triggering-event source" declarative selector if "do X to whoever did Y to me" recurs (not neutral-specific).
- **DR4 ✅ RECORDED — watch-item, no v1 decision.** Going-second comp is HS-exact (P2: 4 cards + The Coin); this game has extra first-mover vectors (press economy, neutral first-crack) but the *direction* is empirical (going second also gets the first intervention window + Coin + card). Ship the HS model + **instrument first/second win-rate from day one** (Game-Server telemetry — data already in the event log; no GameLogic change); LoR alternating-initiative = the recorded escape hatch. No spec change.

Style: the user reframed past my framings twice (DR1 bool→keyword, DR2 OP→consistency) and corrected my fairness misapplication — I verified each step rather than defend the earlier framing ([[feedback-design-fork-style]], [[receiving-code-review]]).

### ⏹ SESSION STOP (2026-06-28, end of session 21)

**State:** Spec-review-3 **DR1–DR4 walked** — DR1 + DR2 APPLIED with substantive spec edits; DR3 collapsed to content (a `spawnPool` reminder note); DR4 recorded as a watch-item. **DR5–DR6 remain.** All G1–G8 done in prior sessions. Spec edits grep-clean; findings doc carries per-DR records + Plan-impact lines.

**▶ RESUME PLAN (session 22):**
1. **DR5** — the one remaining DR with a real spec decision: an **alternate-win-condition seam in ⑧** (make ⑧ consult a small **win-condition predicate list** rather than the hardcoded "hero health ≤ 0" — a cheap-seam-now call like DR1's, so "quest reward = win" / "mill = win" stay expressible without re-touching the settle loop). Present tradeoff + recommend. Read `notes/2026-06-21-spec-review-3.md` DR5.
2. **DR6** — light: `MaxDeathWaves = 16` → treat as a **tunable** + ensure card-load validation catches self-feeders so the runtime cap only bites true bugs, not deep-but-finite legal boards. Likely a one-line note + playtest-watch.
3. **THEN the end-of-pass PLAN RECONCILIATION** — fold every `Plan impact:` list (borrow list + Fireplace + holes + sigils/artifacts + F1–F19 + D1–D4 + spec-review-3 **G1–G8 + DR1–DR6** + library-seam + stale-name sweeps) into `plans/2026-05-27-ccg-game-logic/` → **Epic 01 / T1.1**.

**Git:** session-21 content batch (spec DR1–DR3 edits + findings-doc DR1–DR4 records + this state) committed + pushed to `origin/main`. Design-doc convention = work from `main`.

### Session 20 (2026-06-24): G8 RESOLVED — the pre-halt death rule REMOVED (cadence now uniform)

Resumed at G8 with the user's mandated **Discover/Modal review first**. The review generalized the finding past the session-19 carve-out: the **pre-halt death rule (amendment B)** — force-settle ⑦ before a type-A halt (`PendingChoice` + declaration-hold) — was the **only** place the cascade-settle cadence was violated, and it gave *player choices* a board no other action gets. Three nails, surfaced by the user's own probing (Discover-as-immediate-effect, then chained Discover→Target targeting):
- **Inconsistency:** with the rule, ③′ ≠ ⑥′ for the same responder (one sees dying minions, the other doesn't), and a Discover-and-summon is luckier than a battlecry-summon (the rule frees a board slot the battlecry doesn't).
- **Self-contradiction:** §4 ③ (:1044) already says a `Dead…`/`All…` selector may target a **still-on-board** mortally-wounded minion — but the pre-halt rule removes it before a targeted choice can pick it. The two already fought whenever a choice sat between damage and a targeted effect.
- **Breaks an intended card:** *"When a friendly minion is Mortally Wounded, discover an effect"* (save-via-Discover = R1 through a choice) is silently broken by the rule (the Discover settles the deaths before the responder picks), and re-broken at every link of a chained Discover→Target. Removing the rule makes it work — and it's R1, already committed.

**DECIDED + APPLIED — delete the rule, don't carve it out.** ⑦ death / ⑧ win now run **only** at queue-empty (⑨), uniformly; no halt or window force-settles early. Choices decide against the live (pending-death-inclusive) board, the `All…/Alive…/Dead…` trichotomy deciding offerability — identical to a battlecry. G8(a) ("a response may open a `PendingChoice`") collapses to "it just works" (orthogonal `pendingChoice`/`pendingIntervention` §1 fields, not barred by depth-1; chained choices each their own `PendingChoice`). Verified the **immediate-resolution rule survives** on its own merits (a FIFO-appended save loses its target to the *normal* queue-empty ⑦ — no pre-halt needed). Applied to spec §4: PendingChoice Interruption (rule → uniform-cadence statement + G8(a)/chained-choice note), ⑥′ step 2 (dropped the defunct contrast), "Response resolution" (re-cite the real settle). Grep-clean (remaining `pre-settle`/`pre-halt` hits are the supersede note + the accurate "post-reaction windows never pre-settle"). Findings doc G8 → ✅ RESOLVED + a Plan-impact line added.

Style held: the user drove the generalization (probing Discover-immediate-resolution + chained targeting) **past my carve-out recommendation** — I verified each step rather than defend the earlier ruling ([[feedback-design-fork-style]], [[receiving-code-review]]). The mandated review-before-deciding was the right call: the carve-out would have left the wart in place.

### ⏹ SESSION STOP (2026-06-24, end of session 20)

**State:** **G8 RESOLVED** — the pre-halt death rule is deleted; cadence is uniform (⑦/⑧ only at queue-empty). **All of spec-review-3 G1–G8 are now walked** (G1–G7 session 19, G8 here). **DR1–DR6 (design recs) NOT yet walked.** Spec + findings doc + this state edited; spec grep-clean.

**▶ RESUME PLAN (session 21):**
...
**State:** Spec-review-3 G-findings walked G1–G7 (applied/no-op), **G8 left OPEN by the user's choice.** DR1–DR6 (design recs) not yet walked. All spec edits grep-checked; the findings doc carries per-finding records + a consolidated **Plan impact** section. Style held: present-trade-off-+-recommend; verify HS against sources (G4 flipped on a wiki check); requested artifacts as the centerpiece (the G5 invocation walkthrough, the G6 oscillation/Enrage worked examples). The user twice improved on my recommendation (G2 clean fold, G6 drop-the-keyword) and once corrected my reasoning (G6 Example B) — [[feedback-design-fork-style]].

**▶ RESUME PLAN (session 20):**
1. **G8 FIRST — but precede it with a Discover/Modal review (user's explicit ask):** re-read the **pre-halt death rule** (amendment B; §4:1190 "PendingChoice Interruption" + the §4 cadence) and re-derive *why* it settles ⑦ before a type-A halt, what it protects, and its importance — THEN decide G8 (esp. half (b): does a response-nested `PendingChoice` pre-settle or inherit the reaction's deferred-death state?). The presented ruling is in the findings doc G8 entry; don't apply (b) until the review is done.
2. **Then DR1–DR6** (design-space calls): DR1 (LoR-style burst/non-interceptable window tier — biggest UX/pacing risk) and DR2 (the universal "pay-mana-draw" starter = Life-Tap-without-the-life-cost) are the two worth deciding before content design hardens; DR3 (neutral-lane RNG/first-crack), DR4 (going-second instrumentation), DR5 (alt-win seam in ⑧), DR6 (`MaxDeathWaves=16` tunable) lighter.
3. **Then the end-of-pass PLAN RECONCILIATION** — fold every `Plan impact:` list (borrow list + Fireplace + holes + sigils/artifacts + F1–F19 + D1–D4 + spec-review-3 G1–G8 + library-seam + stale-name sweeps) into `plans/2026-05-27-ccg-game-logic/` → **Epic 01 / T1.1**.

**Git:** session-19 work committed + pushed to `origin/main`. Working from `main` (design-doc convention).

### Session 18 (2026-06-21): F19 RESOLVED — the SIMPLIFICATION (sub-drain rejected & reverted)

**The user rejected the session-17 F19 "sub-draining" mechanism as unclean; we redesigned and RESOLVED it this session.** The turning point was the user's question *"what places the body on board?"* — which exposed that the sub-drain had deferred the **`MinionSummonedEvent` publish** (not just the trigger registration), leaving the placed body with **no announcing event** (a Battlecry `DealDamageAction` would carry `sourceId = Newcomer` before any event introduced it — violating §2 "every state change is announced" / §3 `IActionQueue`). That dissolved the whole mechanism.

- **Resolution — use the existing summon flow.** `ResolveCardAction` ④ does the **ordinary summon** (place body + register `OnSummon` + publish `MinionSummonedEvent`, all inline, *identical to a token's `SummonMinionAction`*) and then **enqueues the Battlecry after**. No sub-drain, no `FinalizeSummon`, no deferred epoch — an entire mechanism deleted.
- **Why it works:** the Battlecry is enqueued at ④ **before** `MinionSummonedEvent` publishes at ⑤, so it sits ahead of summon-reactions → resolves first → **(iii)** other minions' "after you summon" reactions still fire after the Battlecry (HS-faithful); **(i)** Battlecry stays intervenable (its actions keep ③′/⑥′); **on-time placement** (event before Battlecry, no dangling `sourceId`).
- **The trade — line 731 / requirement (ii) DROPPED as a documented HS divergence.** Triggers are live at summon, so a minion **does** react to its own Battlecry's events. **HS rule verified 2026-06-21** (Advanced Rulebook: an HS minion never triggers off its own Battlecry) — a real, deliberate divergence, accepted for **engine uniformity** (no "summoned-but-dormant" sub-state; triggers live by board zone like everything else). Narrow blast radius: only a minion's own `ITrigger`s on its own Battlecry; own-summon-event exclusion (equal-epoch), keyword hooks (pulled), Deathrattle (snapshot) all unaffected. Reversible path back to HS-faithfulness (`FinalizeSummonAction` + DFS via generalized `EnqueueFront`) recorded in spec **Unaddressed Features → "A minion triggers off its own Battlecry."**
- **Honest weighting (user asked for it):** recommended the simplification primarily on the **consistency** argument — line 731 was the *only* place needing a dormant-trigger sub-state, a foreign HS import fighting the engine's pull-don't-store / everything-is-an-action ethos — secondarily simplicity; "easier for players" judged a wash with a caveat (the own-summon-excluded-but-own-Battlecry-included boundary is subtle; the self-trigger is also a design *opportunity*, not just a footgun). Also stress-tested: summon-triggered removal does **not** fizzle the Battlecry (enqueued ahead + minion lingers to ⑦).
- **Applied to spec (grep-clean):** §2A `ResolveCardAction` ④, §3 line 731 (trigger-timing note), §1 `MinionOnBoard` TRIGGERS comment, §3 `IEventBus` note, + new Unaddressed-Features entry. Findings doc F19 → ✅ RESOLVED (sub-drain block marked SUPERSEDED/REVERTED) + walk-progress updated.

**D1–D4 DONE (same session) — spec-review-2 is now COMPLETE.**
- **D1 ✅ applied** — one-line corollary at §3:786: the active player has no ③′/⑥′ window on their own turn ⇒ self-inflicted lethal on your own turn (own AoE clipping your hero, self-damage, fatigue) is unsaveable by reactive cards (HS-consistent: Ice Block is off-turn). User confirmed in line with expectations.
- **D2 ✅ resolved — and the original premise was overturned.** Original finding: "`SpellCastEvent` fires at commitment even if countered (cast-triggers fire for a countered spell)." The user challenged the consistency vs `MinionSummonedEvent` (which fires at *resolution*). **HS re-verified (Advanced Rulebook + Counterspell wiki, 2026-06-21): current HS suppresses ALL cast triggers on a countered spell** (Patch 11.2.0, 2018 — the pre-2018 "whenever you cast" early-fire was patched out). So `SpellCastEvent` **moved `PlayCardAction` ④ (commit) → `ResolveCardAction` ④ (resolution)** — the spell-side analog of `MinionSummonedEvent`. A countered spell fires `CardPlayedEvent` (played, mana spent, → graveyard) but no `SpellCastEvent`. User's follow-up clinched that no new event is needed: the **pre**-resolution hook already exists as the ③′ `ActionDeclaredEvent` (the interception/Counterspell phase), and cast-synergy is a **post**-reaction → `SpellCastEvent`. Touch-points: §2A `PlayCardAction`/`ResolveCardAction` ④, §2B `SpellCastEvent` row, the "Fireball countered" trace example, §3:786.
- **D3 ✅ no edit** (subsumed by F5's `rebornAvailable = true`). **D4 ✅ no edit** (press cap = balance lever for tuning).

**Then started SPEC REVIEW #3 (user-requested) — a third full read-only review.** Findings written to **`notes/2026-06-21-spec-review-3.md`** (NOT yet walked/applied — input queue, like reviews #1/#2). **8 findings (G1–G8)** + a **"Checked and consistent"** coverage section + a **6-item prior-art game-design-recommendations chapter (DR1–DR6, web-grounded)**. Headliners:
- **G1 [BUG/M] — Combo counts its own card:** `cardsPlayedThisTurn++` at `PlayCardAction` ④ (commit) precedes the Combo battlecry's eval at `ResolveCardAction` ④ (resolve), so the card self-counts; the predicate (never specified) must be `≥ 2`, not `> 0`.
- **G2 [HOLE/M→H] — `SpellCastEvent` lacks the cast card:** carries only `cardId` (sibling `CardPlayedEvent` carries full `card`); by publish the spell is a `GraveyardSpell` (definitionKey only), so cost/stat-gated cast-synergy can't read `effectiveCost`. D2 sharpened it — "cast a 5+ spell that resolves" is inexpressible. Fix: put `card` on the event.
- **G3–G8 [L]:** tribe fields missing from the `MinionOnBoard` block; `TakeControlAction` "reject" should be "fizzle"; `InterventionPlayedEvent`/`InterventionWindowClosedEvent` look orphaned; Enrage bonus storage under-specified; setup-artifact registration epoch unstated; `PendingChoice` inside a responder's invocation unaddressed.
- **DR1–DR6:** window-pacing needs an LoR-style burst/non-interceptable tier (biggest UX risk); the universal "pay-mana-draw" starter is Life-Tap-without-the-life-cost given to everyone (add a real cost); neutral-lane RNG/first-crack tuning; going-second instrumentation; alt-win seam in ⑧; revisit `MaxDeathWaves=16`. F19 + D2 + the epoch model + directed-events all re-verified CLEAN.

**▶ RESUME (next session, after the user has reviewed the review-3 doc): WALK the G/DR findings one-by-one** (present-tradeoff-+-recommend, the reviews-#1/#2 cadence). User's lean: G1+G2 are the substantive ones; most of G3–G8 are one-liners; DR1+DR2 are the design calls worth deciding before content design hardens. **THEN** the end-of-pass **PLAN RECONCILIATION** (fold every `Plan impact:` list — borrow list + Fireplace + holes + sigils/artifacts + F1–F19 + D1–D4 + any applied G-findings + library-seam + stale-name sweeps — into `plans/2026-05-27-ccg-game-logic/`) → **Epic 01 / T1.1**.

### ⏹ SESSION STOP (2026-06-21, end of session 18)

**State:** Long session. (1) **F19 RESOLVED** via the simplification (sub-drain rejected + reverted) — committed + pushed `5a5c547`. (2) **D1–D4 applied → spec-review-2 COMPLETE** (D2 flipped on HS re-verification: `SpellCastEvent` moved commit→resolution). (3) **Spec review #3** produced (8 findings + 6 design recs, in `notes/2026-06-21-spec-review-3.md`) — **awaiting the user's read before we walk it.** No G/DR finding applied.

**Git:** F19 batch already on `origin/main` (`5a5c547`). This stop commits the **D1–D4 batch** (spec + spec-review-2 doc + this session-state) **and the new spec-review-3 doc**. Working from `main` (design-doc convention).

**Style (carried):** present-tradeoff-+-recommend, verify HS against sources not memory (D2 + F19 both turned on web-verified HS rules — the F18 lesson held), requested artifacts (the review doc) are the centerpiece reply — see [[feedback-design-fork-style]], [[feedback-deliverable-visibility]].

### ⏹ SESSION STOP (2026-06-20, end of session 17)

> **⚠ SUPERSEDED by session 18 (2026-06-21):** the F19 "sub-drain" decided + applied below was **rejected by the user and reverted** — F19 is now resolved via the *simplification* (see the session-18 block above). The "do NOT commit" hold below is also lifted (the user has reviewed). This block is retained as the historical record of session 17.

**State:** Session 17 = a short resume that **decided + applied F19** (the finding session 16 stopped mid-discussion on) and then **teed up D1–D4** for confirmation. **STOPPED before D1–D4** at the user's request — they want to **review the F19 spec edits once more and discuss D1–D4 at a later time.** **⚠ ALL session-17 edits are UNCOMMITTED in the working tree** (spec + findings doc + this stop block) — **do NOT commit or push** until the user has reviewed.

**F19 [M, HOLE, NEW] → DECIDED + APPLIED (option a, summon event deferred).** Battlecry/minion-summon ordering was under-pinned vs. the per-action epoch model: Battlecry effects are later-epoch enqueued actions, so the natural "`OnSummon` at the summon's ④" reading (birthEpoch = summon epoch) lets a freshly-played minion react to its *own* Battlecry — violating §3 line 731. **Two calls decided:**
- **Call 1 → option (a), honor line 731 (not drop it).** `ResolveCardAction` ④ runs a minion card in three steps: **(1) place the body** (on board for the Battlecry's targeting/AoE; `MinionSummonedEvent` NOT yet published); **(2) run the Battlecry**, whose effect actions **sub-drain to completion as ordinary fully-windowed queue citizens** — each keeps its ③′/⑥′, so the Battlecry is **intervenable, never swallowed** (the user's explicit requirement this session); **(3)** after the sub-tree drains, publish `MinionSummonedEvent` + register `OnSummon` triggers/Deathrattle/keyword-hooks at a **fresh post-Battlecry epoch**. The deferred epoch is what makes the creation-epoch filter actually deliver 731 (minion's `birthEpoch` > every Battlecry sub-action's `originEpoch` ⇒ no self-reaction). Rider: minion removed during the Battlecry ⇒ step 3 skipped.
- **Call 2 → `MinionSummonedEvent` publishes AFTER the Battlecry** (reversed my earlier "before/placement" lean). HS-faithful per **Razorfen Hunter (Battlecry: summon a Boar) + Knife Juggler** — HS summons the Boar first, *then* throws the knives ⇒ summon-triggers resolve after the Battlecry. Body still *placed* before (board presence/AoE); only the event *publish* defers. Co-located with `OnSummon` at the same post-Battlecry epoch ⇒ the new minion doesn't react to its own summon event either (equal-epoch exclusion). Also mechanically cleaner (HS order falls out of the epoch stamp, no sub-queue assumption). **HS claim flagged for verification per the F18 lesson** — user accepted on the Razorfen evidence; worth re-confirming on review.

**Spec touch-points applied (grep-clean):** §2A [`ResolveCardAction` ④ :319](docs/superpowers/specs/2026-05-26-game-mechanics.md#L319) (three-step order + token-path contrast); §3 [line 731](docs/superpowers/specs/2026-05-26-game-mechanics.md#L731) (mechanism, not just intent); §1 [`MinionOnBoard` TRIGGERS comment :111](docs/superpowers/specs/2026-05-26-game-mechanics.md#L111); §3 [epoch invariant :548](docs/superpowers/specs/2026-05-26-game-mechanics.md#L548) (deferred subscribe stamps the *advanced* post-Battlecry `currentActionEpoch`, not `ResolveCardAction`'s own — the load-bearing point). Findings doc F19 → ✅, walk pointer advanced to D1–D4.

**▶ RESUME PLAN (session 18):**
1. **User reviews the F19 edits** (esp. the Razorfen/Knife-Juggler HS-ordering claim) — adjust if their HS recall differs. Then **D1–D4** confirm-on-record (presented this session, NOT yet applied): **D1** = active player can't reactively save self on own turn (by design; HS-consistent — recommend a 1-line note); **D2** = `SpellCastEvent` fires at commitment even if the spell is then countered (HS Counterspell — recommend a 1-line note at `CardPlayedEvent`/`SpellCastEvent`); **D3** = Reborn simplification (subsumed by F5, no edit); **D4** = press cap of 3 is a balance lever (no edit). My rec: add the D1+D2 one-liners, none for D3/D4.
2. **Then spec-review-2 is COMPLETE → end-of-pass plan reconciliation** — must fold in F1–F17 + F19 (esp. F4's `IEntityPredicate`, F1 mana seeding, F2/F3 intervention model, F5 Reborn, F9 ChoiceType/ChoiceOption, F10 fizzle-RFG) on top of the prior backlog → **Epic 01 / T1.1**.

**Git:** working tree has **uncommitted** session-17 edits (per user). Nothing pushed. Prior sessions' work (through session 16) is already on `origin/main`.

### ⏹ SESSION STOP (2026-06-19, end of session 16)

**State:** Session 16 continued the **spec-review-2 walk** (`notes/2026-06-17-spec-review-2.md`). Decided + applied **F9 + F10** (the two findings session 15 had parked), then batch-applied **F11–F17**. Raised and **retracted F18**. **Stopped mid-discussion of a NEW finding, F19** (Battlecry/summon ordering) — *not decided, not applied*. **D1–D4 still not applied.** Read the findings doc's "▶ Walk progress" block + the **F19** section FIRST on resume.

**Applied to spec this session (grep-clean, F-tags trace to the findings doc):**
- **F9 [M, HOLE] → lock-now.** Added to §1 Supporting types: `enum ChoiceType { Discover, Target, Modal }` (mulligan is NOT a member — rides `SubmitMulliganAction`) + `record ChoiceOption(string optionId, string? entityId = null, string? definitionKey = null, string? label = null)`. **Dropped `choiceId`** from `SubmitChoiceAction` → `{ playerId, selectedOptionId }`. Baked the invariant: `ChoiceOption` = a **display/answer-routing token**, never the effect; `optionId` = the stable per-mode dispatch key the continuation switches on (a Modal mode's effect-spec lives in the card definition's `modes[]`, reached by `optionId`). User probed hard (the Modal "where does the number 2 come from" question → answer: the card definition, via the continuation) and asked which option **maximizes design space** → lock-now (record already maximal: all-optional fields + unconstrained `PendingChoice.context` continuation ⇒ a new mechanic never needs a new option field). Three worked patterns (Discover/Target/Modal) with payloads were the centerpiece reply.
- **F10 [M, DESIGN] → option a.** A fizzled (countered) **minion** card is **removed from the game** — no `GraveyardMinion` entry (never reached board / never died ⇒ not a resurrection target; no `diedOnTurn` ambiguity; no tracked exile zone needed; HS-faithful). §2A `ResolveCardAction` fizzle branch.
- **F11–F17 [L, wording] batch.** F11 Battlecry "PlayCard pipeline" → "card-resolution pipeline (`ResolveCardAction` ④)". F12 directed-events off-by-one — `MulliganStartedEvent` is directed **per-player with NO public sibling**; the other five each pair with a sibling. F13 `turn.number++` added at Turn Lifecycle **step 6** (steps 1–5 deliberately read the ending turn's number); Match Setup **step 8** seeds `turn.number = 1` + `activePlayerId`. F14 `EntityId` = documented **alias for `string`** in v1 (typed wrapper = possible Epic-01 refinement) — §2A ID-convention note. F15 disambiguated `autoSkipAll` (standing, all windows, turn-spanning) vs `SkipAll` (one action's ③′+⑥′) at both sites. F16 savability caveat — only a **damage-domain** wound (`currentHealth ≤ 0 < maxHealth`) is **heal**-saveable; a **maxHealth-collapse** wound (`maxHealth ≤ 0`) is not (heal caps at ≤0) — "savable" ≠ "heal-saveable". F17 `SpellDamageBonus` → `spellDamageBonus` (5 sites).
- **F16-adjacent prose tightening (§1 line 95):** replaced the misleading "*existing damage persists*" with the correct **clamp-down/never-kills** wording; sole death path = `maxHealth ≤ 0` from a **negative** buff/aura.

**F18 — RAISED then RETRACTED (no formula change).** Mid-F16 I *wrongly* claimed HS kills a damaged, health-buffed minion on buff removal and proposed a "signed-Δ" fix to the #13 fall formula. **The user corrected the HS rule: removing a buff NEVER kills — current Health drops only to the printed stat, and if already below it, is unchanged.** That is exactly what the existing `min(currentHealth, newMaxHealth)` already does, so the spec was already HS-correct; the signed-Δ fix would have *introduced* a divergence. Formula unchanged (`min(currentHealth + max(0, Δmax), newMax)`); only the prose tightening above was applied. *(Lesson saved: verify HS rules before asserting them.)*

**⏳ STOPPED AT F19 [M, HOLE, NEW] — UNDER DISCUSSION (user paused the session here).** Battlecry/minion-summon ordering is under-pinned vs. the per-action epoch model. **The seam:** Battlecry effects are *enqueued actions* (a "Battlecry: deal 3" is a real `DealDamageAction` with its own ③′/⑥′ windows — the all-damage-one-channel rule), so they run at **later epochs** than the summon (epoch N). The epoch filter stamps a subscriber's `birthEpoch` with its registering-action's epoch; the natural "`OnSummon` at the summon's ④ (epoch N)" reading gives the minion `birthEpoch = N`, and `N < N+1` ⇒ the minion **would** react to its own Battlecry's sub-actions — violating line 731 ("Battlecry before the minion's persistent triggers go live"). For 731 to hold, `OnSummon` must be **deferred until the Battlecry sub-tree fully drains** and stamped a **post-Battlecry epoch** — neither the synchronous sub-drain nor the deferred epoch is specified. Plus an unstated sub-question: does `MinionSummonedEvent` fire at **placement (before Battlecry)** or after?
**My recommendation (presented, awaiting user):** option **(a)** — place body + emit `MinionSummonedEvent` → Battlecry **sub-drains to completion** (response-resolution idiom) → **then** `OnSummon` registers triggers with a fresh post-Battlecry epoch. Alt = drop line 731 (accept self-Battlecry triggering). **Two open calls:** (1) confirm (a) vs drop 731; (2) where `MinionSummonedEvent` fires. Full write-up = findings doc **F19** section.

**▶ RESUME PLAN:**
1. **Resume the F19 discussion** (findings-doc F19 section has everything): decide the two open calls, apply to §2A `ResolveCardAction` ④ + §3 line 731 (+ the token-summon `SummonMinionAction` path differs in `OnSummon` timing — state it).
2. **Then D1–D4** confirm-on-record notes (none applied; the D1 edit was drafted but **rejected** so the user could raise the F16/F18 then F19 questions). D1 = active player can't reactively save self on own turn (by design); D2 = `SpellCastEvent` fires at commitment even if the spell is then countered; D3 = Reborn simplification (subsumed by F5, no edit); D4 = press cap of 3 is a balance lever.
3. **Then spec-review-2 is COMPLETE → end-of-pass plan reconciliation** — now must ALSO fold in F1–F17 + the F19 resolution (esp. F4's `IEntityPredicate`, F1 mana seeding, F2/F3 intervention model, F5 Reborn, F9 ChoiceType/ChoiceOption, F10 fizzle-RFG) on top of the prior backlog → **Epic 01 / T1.1**.

**Git:** Session-16 spec edits (F9–F17 + the §1 line-95 tightening) + the findings-doc updates + this stop block committed and pushed to `origin/main` at session end. The spec is in a **coherent** state — F9–F17 are fully applied and internally consistent; F19 is a *flagged open question*, not a half-applied edit; the rejected D1 edit was never written. Working from `main` (design-doc convention).

**Note on style (carried):** walked findings one-by-one, present-tradeoff-+-recommend; user repeatedly probed for worked examples + prior-art before deciding, and on F18 corrected a wrong HS assertion of mine — verify HS rules against fact, not memory (see [[feedback-design-fork-style]], [[feedback-deliverable-visibility]]).
