...
**Remaining pre-implementation work:** the **end-of-pass plan reconciliation** (all `Plan impact:` lists → epic/ticket files; weapon-scope deletion + artifact re-scope + sigil additions + the library-seam constraint + `HasTribe`; stale-name sweeps) → then **Epic 01 / T1.1**. Still deferred-by-design (have homes): R2 inversion stat-math, marked-for-destruction scope.
