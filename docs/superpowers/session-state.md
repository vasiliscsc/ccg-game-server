...
**State:** session-5 Hole #3 + follow-on spec edits + bookkeeping. Hole #3 committed + pushed (`3adc01f`); the summoning-sickness follow-on is applied but **commit pending** at time of writing. Spec re-grepped: no stale `canAttack`/`originalCard`/"Taunt ignored" language (remaining mentions are deliberate supersede/rationale notes).
